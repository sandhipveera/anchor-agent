...
the appliances he actually has, and navigate a job search — on his terms. The
agent reasons with **Gemini**, remembers with **Elastic**, persists state in
**Firestore**, and acts on the outside world **only with his explicit approval of
the exact action.** He is the user, and he is in charge.
...
location tracking. No mood/emotion detection. No social features. No
money-management (deferred). No time-shaming. No auto-advance / auto-detect /
auto-escalate. No crisis hotlines as a runtime path. No audio recording — writing
samples are text only.
...
- **Job Search slice** — a JD decoder (Gemini Pro extracts must-haves,
  negotiables, red-flag phrases, and autism-friendly signals) and an application
  drafter that writes in his own register. **The agent never sends.**
...
[Android PWA]  ──HTTPS──►  [Cloud Run: FastAPI]
   ...
   no sound · no voice           ├─► [Vertex AI ADK on Agent Engine] ─► Gemini 2.5 Flash/Pro
                                 ├─► [Elastic Serverless · Agent Builder MCP] ─► repertoire/memory/knowledge
                                 ├─► [Firestore] ─► current_task · profile · settings (per-uid)
...
- Logs are **scrubbed of personal content** — no task text, pantry, JD text, or
  message bodies in any log line. No verbose access logging.
...
├── LICENSE                  # Apache 2.0
├── README.md                # this file
├── DESIGN_PRINCIPLES.md     # the 15 immutable principles (public-facing)
...
└── docs/
    ...
    └── seed-data/           # meals, employers, lifeline messages
```
